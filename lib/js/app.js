...
Generator.prototype = {
	...
	start: function() {
		...
		this._repInt = setInterval(function(){
			getRandomNoteDuration(thisGenerator.bpm,thisGenerator.subdivision);
		},1000);
	},
	stop: function() {
		//var thisGenerator = this;
...
var myGenerator = new Generator(oscillator,60)
...
function getRandomNoteDuration(bpm,subdivision) {
	...
	console.log(lengthOfOneBeat);
	// Convert the subdvision (16th note, 8th, etc) into a number to be used in random
	var subdivMax = Math.sqrt(subdivision) - 1;

	// Get a random power of 2 -- 1,2,4,8,16
	var randomDivision = Math.pow(2,Math.floor(Math.random() * (subdivMax - 0 + 1)) + 0);
